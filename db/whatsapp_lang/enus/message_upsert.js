...
module.exports = async function handleMessageUpsert({ data, msgOriginal, server_url, apikey, instance }) {
    ...
    function buildMenu() {
        return [
            '╭─🎲─ *MAIN MENU* ────',
            `│ • \`${mainPrefix}play <query>\` - search music/video`,
            `│ • \`${mainPrefix}sticker\` - create sticker`,
            `│ • \`${mainPrefix}sfundo\` - sticker without background`,
            `│ • \`${mainPrefix}tomp3\` - convert to MP3`,
            `│ • \`${mainPrefix}ytmp4 <link>\` - download YouTube video`,
            `│ • \`${mainPrefix}tiktok <link>\` - download TikTok video`,
            `│ • \`${mainPrefix}instagram <link>\` - download Instagram media`,
            `│ • \`${mainPrefix}facebook <link>\` - download Facebook video`,
            `│ • \`${mainPrefix}kwai <link>\` - download Kwai video`,
            `│ • \`${mainPrefix}id\` - show chat/group ID`,
            `│ • \`${mainPrefix}piada [topic]\` - tell a joke`,
            `│ • \`${mainPrefix}brincadeiras\` - list games`,
            `│ • \`${mainPrefix}figurinhas\` - send stickers`,
            `│ • \`${mainPrefix}tabela\` - show table`,
            `│ • \`${mainPrefix}sorteio\` - create giveaway`,
            `│ • \`${mainPrefix}sorteio2\` - simple giveaway`,
            `│ • \`${mainPrefix}enquete\` - create poll`,
            '│',
            '│ 🔒 *Admin Commands*',
            `│ • \`${mainPrefix}menuadm\` - admin options`,
            '╰──────────────────────'
        ].join('\n');
    }
    ...
    if (isGroup && !botRegistrado && isCommand) {
        const allowed = ['menu', 'm', 'menuadm', 'menuadmin', 'pt', 'en', 'es'];
        if (cleanCommand && !allowed.includes(cleanCommand)) {
            await sendText(
                server_url,
                apikey,
                instance,
                remoteJid,
                `To use all functions, register the bot at ${basesiteUrl}.`,
                id
            );
            return;
        }
...
só analise com cuidado cada resposta`;
    ...
    if (isGroup && !isCommand) {
        ...
        if (insult) {
            await sendText(server_url, apikey, instance, remoteJid, 'cuck is your dad, you son of a bitch', id);
            return;
        }
        if (called) {
            const prefixInfo = prefixList.join(' ');
    ...
    if (isCommand) {
        ...
        switch (cleanCommand) {
            ...
            case 'menuadmin':
                ...
                const comandosAdmin = [
                    '╭──────────────',
                    '┇ *乂 A D M I N   M E N U 乂*',
                    '╰──────────────\n',
                    '🛠️ *GROUP MANAGEMENT*',
                    `\`${mainPrefix}ban @user\` – Remove mentioned member`,
                    `\`${mainPrefix}apagar\` – Delete quoted message`,
                    `\`${mainPrefix}permitirlink <link>\` – Add link/domain to whitelist`,
                    `\`${mainPrefix}removerlink <link>\` – Remove link/domain from whitelist`,
                    '⚙️ *BOT SETTINGS*',
                    `\`${mainPrefix}botinterage\` – Enable AI replies in group`,
                    `\`${mainPrefix}antilink\` – Delete non-whitelisted links`,
                    `\`${mainPrefix}banextremo\` – Ban users who send forbidden links`,
                    `\`${mainPrefix}antilinkgp\` – Delete group invites`,
                    `\`${mainPrefix}bangringos\` – Remove non-Brazilians`,
                    `\`${mainPrefix}soadm\` – Only admins use commands`,
                    `\`${mainPrefix}autosticker\` – Convert image/video to sticker`,
                    `\`${mainPrefix}autodownloader\` – Auto download media`,
                    `\`${mainPrefix}modobrincadeira\` – Toggle fun commands`,
                    '🖼️ *CUSTOMIZATION*',
                    `\`${mainPrefix}fundomenu\` – Set/remove menu background`,
                    `\`${mainPrefix}fundobemvindo\` – Set/remove welcome image`,
                    `\`${mainPrefix}legendabemvindo <text>\` – Update welcome caption`,
                    // add other administrative commands here…
                ].join('\n');
            ...
            case 'fundomenu':
                if (!isGroup || !isAdmin) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ This command can only be used by administrators in groups.', quotedId);
                    break;
                }
                ...
                if (args[0]?.toLowerCase() === 'reset') {
                    try {
                        ...
                        await sendText(server_url, apikey, instance, remoteJid,
                            '✅ Menu background removed. The default background will be used.', quotedId);
                        await sendReaction(server_url, apikey, instance, { remoteJid, id }, '♻️');
                    } catch (err) {
                        if (err.code === 'ENOENT') {
                            await sendText(server_url, apikey, instance, remoteJid,
                                '⚠️ No custom background was set.', quotedId);
                        } else {
                            console.error('Erro ao remover fundo do menu:', err.message);
                            await sendText(server_url, apikey, instance, remoteJid,
                                '❌ Error trying to remove the menu background.', quotedId);
                        }
                    }
                    break;
                }
                ...
                if (!quotedMessageId) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        `📌 Send \`${mainPrefix}fundomenu\` replying to an image.\nOr \`${mainPrefix}fundomenu reset\` to remove the custom background.`, quotedId);
                    break;
                }
                ...
                try {
                    ...
                    if (!media?.base64 || !media.mimetype.startsWith('image/')) {
                        await sendText(server_url, apikey, instance, remoteJid,
                            '⚠️ The replied message does not contain a valid image.', quotedId);
                        break;
                    }
                    ...
                    await sendText(server_url, apikey, instance, remoteJid,
                        '✅ Menu background updated successfully!', quotedId);
                    await sendReaction(server_url, apikey, instance, { remoteJid, id }, '🖼️');

                } catch (err) {
                    console.error('Erro ao salvar novo fundo do menu:', err.message);
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ Error trying to save the new background image.', quotedId);
                }
                break;
            ...
            case 'fundobemvindo':
                ...
                try {
                    ...
                    if (!bot) {
                        await sendText(server_url, apikey, instance, remoteJid, '❌ Bot not found in database.', quotedId);
                        break;
                    }
                    ...
                    if (args[0]?.toLowerCase() === 'reset') {
                        ...
                        await sendText(server_url, apikey, instance, remoteJid,
                            '♻️ Welcome image removed. The default background will be used.', quotedId);
                        await sendReaction(server_url, apikey, instance, { remoteJid, id }, '✅');
                        break;
                    }
                    ...
                    if (!quotedMessageId) {
                        await sendText(server_url, apikey, instance, remoteJid,
                            `📎 Send \`${mainPrefix}fundobemvindo\` replying to an image.\nOr use \`${mainPrefix}fundobemvindo reset\` to remove it.`, quotedId);
                        break;
                    }
                    ...
                    await sendText(server_url, apikey, instance, remoteJid,
                        '✅ Welcome image updated successfully!', quotedId);
                    await sendReaction(server_url, apikey, instance, { remoteJid, id }, '🖼️');

                } catch (err) {
                    console.error('Erro no fundobemvindo:', err.message);
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ Error processing the welcome background command.', quotedId);
                }
                break;
            ...
            case 'bemvindo':
                if (!isGroup || !isAdmin) {
                    await sendReaction(server_url, apikey, instance, { remoteJid, id }, '❌');
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ Only administrators can enable or disable this feature.', quotedId);
                    break;
                }
                ...
                if (!allowedCommands.bemvindo) {
                    ...
                    await sendText(
                        ...
                        instance,
                        remoteJid,
                        '⚠️ This feature is not available on your current plan. Access the dashboard to upgrade.',
                        quotedId
                    );
                    break;
                }
                ...
                try {
                    const bot = await BotConfig.findOne({ groupId: remoteJid });
                    if (!bot) {
                        await sendText(server_url, apikey, instance, remoteJid, '❌ Group not found in database.', quotedId);
                        break;
                    }
            ...
            case 'legendabemvindo':
                ...
                try {
                    ...
                    if (args[0]?.toLowerCase() === 'reset') {
                        ...
                        await sendText(server_url, apikey, instance, remoteJid,
                            '✅ Welcome caption cleared successfully.', quotedId);
                        await sendReaction(server_url, apikey, instance, { remoteJid, id }, '🗑️');
                        break;
                    }
                    ...
                    if (!novaLegenda) {
                        await sendText(server_url, apikey, instance, remoteJid,
                            `✏️ Use \`${mainPrefix}legendabemvindo <text>\` or reply to a message with the desired caption.\nExample: \`${mainPrefix}legendabemvindo Welcome to the group! 🎉\``, quotedId);
                        break;
                    }
                    ...
                    await sendText(server_url, apikey, instance, remoteJid,
                        '✅ Welcome caption updated successfully!', quotedId);
                    await sendReaction(server_url, apikey, instance, { remoteJid, id }, '✍️');

                } catch (err) {
                    console.error('Erro ao atualizar legenda de boas-vindas:', err.message);
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ Error updating the welcome caption.', quotedId);
                }
                break;
            ...
            case 'permitirlink':
                ...
                if (args.length === 0) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        `⚠️ Correct usage: \`${mainPrefix}permitirlink <link or domain>\`\nE.g.: \`${mainPrefix}permitirlink youtube.com\` or \`${mainPrefix}permitirlink https://bet.com/myurl\``,
                        quotedId);
                    break;
                }
                ...
                try {
                    ...
                    if (jaTem) {
                        await sendText(server_url, apikey, instance, remoteJid,
                            `ℹ️ The link/domain \`${dominio}\` is already whitelisted in this group.`, quotedId);
                        break;
                    }
                    ...
                    await sendText(server_url, apikey, instance, remoteJid,
                        `✅ Link/domain \`${dominio}\` added to the whitelist successfully.`, quotedId);
                    await sendReaction(server_url, apikey, instance, { remoteJid, id }, '✅');
                } catch (err) {
                    console.error('Erro no comando !permitirlink:', err.message);
                    await sendText(server_url, apikey, instance, remoteJid, '❌ Error adding the link.', quotedId);
                }
                break;
            ...
            case 'removerlink':
                ...
                if (args.length === 0) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        `⚠️ Correct usage: \`${mainPrefix}removerlink <link or domain>\`\nE.g.: \`${mainPrefix}removerlink youtube.com\``, quotedId);
                    break;
                }
                ...
                try {
                    ...
                    if (novo === anterior) {
                        await sendText(server_url, apikey, instance, remoteJid,
                            `⚠️ Link/domain \`${dominio}\` was not in the whitelist.`, quotedId);
                        break;
                    }
                    ...
                    await sendText(server_url, apikey, instance, remoteJid,
                        `🗑️ Link/domain \`${dominio}\` successfully removed from the whitelist.`, quotedId);
                    await sendReaction(server_url, apikey, instance, { remoteJid, id }, '🗑️');
                } catch (err) {
                    console.error('Erro no comando !removerlink:', err.message);
                    await sendText(server_url, apikey, instance, remoteJid, '❌ Error removing the link.', quotedId);
                }
                break;
            case 'linkspermitidos':
                if (!isGroup) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ This command can only be used inside groups.', quotedId);
                    break;
                }
                ...
                try {
                    ...
                    if (!bot) {
                        await sendText(server_url, apikey, instance, remoteJid,
                            '❌ Group not found in database.', quotedId);
                        break;
                    }
                    ...
                    if (links.length === 0) {
                        await sendText(server_url, apikey, instance, remoteJid,
                            'ℹ️ No link or domain has been whitelisted in this group yet.', quotedId);
                        break;
                    }

                    const listaFormatada = links.map((link, i) => `*${i + 1}.* \`${link}\``).join('\n');

                    const resposta = `📜 *Whitelisted links/domains in the group:*\n\n${listaFormatada}`;
                    ...
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ Error listing permitted links.', quotedId);
                }
                break;
            ...
            case 'participantes':
                ...
                try {
                    ...
                    if (!sorteioAtivo) {
                        await sendText(server_url, apikey, instance, remoteJid,
                            '⚠️ No active giveaway was found.', quotedId);
                        break;
                    }

                    const { tipo, pergunta, participantes = [], createdAt } = sorteioAtivo;

                    const dataCriacao = new Date(createdAt).toLocaleString('en-US', {
                        dateStyle: 'short',
                        timeStyle: 'short',
                    ...
                    const mensagem = `📊 *Giveaway participants ${tipo}:* ${pergunta}\n` +
                        `📅 Created at: ${dataCriacao}\n\n` +
                        `${listaFormatada}`;
                    ...
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ An error occurred while fetching participants.', quotedId);
                }
                break;
            ...
            case 'tomp3':
                if (!quotedMessageId) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        '⚠️ Use the *!tomp3* command replying to a video or video document.', quotedId);
                    break;
                }
                ...
                try {
                    const media = await downloadMedia(server_url, apikey, instance, remoteJid, quotedMessageId);
                    if (!media?.base64 || !media.mimetype) {
                        await sendText(server_url, apikey, instance, remoteJid, '❌ Invalid or missing media.', quotedId);
                        break;
                    }
                    ...
                    if (!isVideo) {
                        await sendText(server_url, apikey, instance, remoteJid, '⚠️ Only videos are supported for conversion.', quotedId);
                        break;
                    }
                    ...
                    await sendText(server_url, apikey, instance, remoteJid, '❌ Error converting video to MP3.', quotedId);
                }
                break;
            ...
            case 'fixar':
                if (!isGroup) {
                    await sendText(server_url, apikey, instance, remoteJid, '❌ This command can only be used in groups.', quotedId);
                    break;
                }

                if (!quotedMessageId) {
                    await sendText(server_url, apikey, instance, remoteJid, '❌ You need to reply to a message to pin it.', quotedId);
                    break;
                }

                try {
                    await fixarMensagem(server_url, apikey, instance, remoteJid, quotedMessageId);
                    await sendText(server_url, apikey, instance, remoteJid, '📌 Message pinned successfully.', quotedId);
                    await sendReaction(server_url, apikey, instance, { remoteJid, id }, '📌');
                } catch (err) {
                    console.error('❌ Erro ao fixar mensagem:', err.message);
                    await sendText(server_url, apikey, instance, remoteJid, '❌ Error pinning the message.', quotedId);
                    await sendReaction(server_url, apikey, instance, { remoteJid, id }, '❌');
                }
                break;
            ...
            case 'desfixar':
                ...
                try {
                    ...
                    await sendText(server_url, apikey, instance, remoteJid, '❌ Error unpinning the message.', quotedId);
                }
                break;
            ...
            case 'abrirgp':
                ...
                if (!horarioAbrir || (!/^\d{2}:\d{2}$/.test(horarioAbrir) && horarioAbrir !== '0')) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        `❌ Please provide the time in HH:MM format or \`0\` to clear.\nExample: \`${mainPrefix}abrirgp 08:00\` or \`${mainPrefix}abrirgp 0\``,
                        quotedId);
                    break;
                }
                ...
                try {
                    ...
                    const respostaAbrir = horarioAbrir === '0'
                        ? '⛔ Automatic group opening time removed.'
                        : `🕓 Group will be automatically opened at *${horarioAbrir}*.`;
                    ...
                    await sendText(server_url, apikey, instance, remoteJid, '❌ Error saving the time.', quotedId);
                }
                break;
            case 'fechargp':
                if (!isGroup || !isAdmin) {
                ...
                if (!horarioFechar || (!/^\d{2}:\d{2}$/.test(horarioFechar) && horarioFechar !== '0')) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        `❌ Please provide the time in HH:MM format or \`0\` to clear.\nExample: \`${mainPrefix}fechargp 23:30\` or \`${mainPrefix}fechargp 0\``,
                        quotedId);
                    break;
                }
                ...
                try {
                    ...
                    const respostaFechar = horarioFechar === '0'
                        ? '⛔ Automatic group closing time removed.'
                        : `🔒 Group will be automatically closed at *${horarioFechar}*.`;
            ...
            case 'addtabela':
                ...
                if (!isAdmin) {
                    await sendText(
                        server_url, apikey, instance, remoteJid,
                        '🚫 *Only administrators can add to the table.*',
                        quotedId
                    );
                    break;
                }
                ...
                if (args.length === 0) {
                    await sendText(
                        server_url, apikey, instance, remoteJid,
                        '⚠️ *Uso correto:* !addtabela conteúdo da nova linha\n\n📌 Exemplo:\n!addtabela Aula 03 - Terça-feira 14h\n\n🧩 O texto será adicionado ao final da tabela atual.',
                        quotedId
                    );
                    break;
                }
            ...
            case 'sorteio':
                ...
                if (partes.length < 4) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        `❌ Invalid format.\n\n📌 Use: *${mainPrefix}sorteio <limit> | <winners> | <time> | <title>*\n\nExample: ${mainPrefix}sorteio 30 | 2 | 5m | $50 Pix giveaway`,
                        quotedId
                    );
                    break;
                }
                ...
                if (isNaN(maxParticipantes) || isNaN(winnersCount) || maxParticipantes < 1 || winnersCount < 1) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ The first two parameters must be valid numbers.\nExample: `30 | 2`',
                        quotedId
                    );
                    break;
                }
                ...
                if (!matchTempo) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ Invalid time. Use "5m" for minutes, "2h" for hours or "1d" for days.',
                        quotedId
                    );
                    break;
                }
                ...
                const opcoes = [
                    { name: 'Participate', localId: 0 },
                    { name: 'Do not participate', localId: 1 }
                ];
                ...
                try {
                    ...
                    if (!botConfig) throw new Error('BotConfig not found for the group.');
                    ...
                    console.error('❌ Error registering giveaway:', err.message);
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ Failed to register the giveaway.', quotedId);
                    await sendReaction(server_url, apikey, instance, { remoteJid, id }, '❌');
                }
            ...
            case 'sorteio2':
                ...
                if (!titulo) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        `❌ Invalid format.\n\n📌 Use: *${mainPrefix}sorteio2 <amount> | <title>*\nExample: \`${mainPrefix}sorteio2 2 | $50 Pix\`\nOr just: \`${mainPrefix}sorteio2 $50 Pix\``,
                        quotedId
                    );
                    break;
                }
                ...
                if (isNaN(quantidade) || quantidade < 1) {
                    await sendText(server_url, apikey, instance, remoteJid, '❌ Invalid number of winners.', quotedId);
                    break;
                }
                ...
                try {
                    ...
                    if (membros.length === 0) {
                        await sendText(server_url, apikey, instance, remoteJid, '❌ No participants found in the group.', quotedId);
                        break;
                    }
                    ...
                    console.error('❌ Error in sorteio2:', err);
                    await sendText(server_url, apikey, instance, remoteJid, '❌ An error occurred while running the giveaway.', quotedId);
                    await sendReaction(server_url, apikey, instance, { remoteJid, id }, '❌');
                }
                break;
            ...
            case 'enquete':
                ...
                if (!tituloEnquete || opcoesEnquete.length < 2) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        `❌ Invalid format.\n\n📌 Use: *${mainPrefix}enquete title | option 1 | option 2 | ...*\nExample: \`${mainPrefix}enquete Favorite prize? | House | Car | Bike\``,
                        quotedId
                    );
                    break;
                }
                ...
                try {
                    ...
                    await sendText(server_url, apikey, instance, remoteJid, '❌ An error occurred while creating the poll.', quotedId);
                    await sendReaction(server_url, apikey, instance, { remoteJid, id }, '❌');
                }
            ...
            case 'abrirgrupo':
                if (!isAdmin) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        '🚫 *Only administrators can use this command.*', quotedId);
                    break;
                }
                ...
                try {
                    ...
                    await sendText(server_url, apikey, instance, remoteJid,
                        '✅ Group window opened successfully on the bot client.', quotedId);
                    await sendReaction(server_url, apikey, instance, { remoteJid, id }, '📂');
                } catch (err) {
                    console.error('❌ Error opening group window:', err.message);
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ Could not open the group window.', quotedId);
                    await sendReaction(server_url, apikey, instance, { remoteJid, id }, '❌');
                }
                break;
            ...
            case 'vencimento':
                ...
                const vencimentoData = planoVencimento
                    ? new Date(planoVencimento).toLocaleString('en-US', {
                        dateStyle: 'short',
                        timeStyle: 'short'
                    })
                    : '⚠️ `Date not set`';

                const mensagemVenc = `⏳ *Group plan expiration:*\n\n📅 *${vencimentoData}* `;
            ...
            case 's':
                if (quotedMessageId) {
                    if (['image', 'video'].includes(quotedType) || isQuotedGif) {
                        ...
                        await sendText(server_url, apikey, instance, remoteJid,
                            '⚠️ Reply to an image or video to create a sticker.', id);
                    }
                    break;
                }
                ...
                await sendText(server_url, apikey, instance, remoteJid,
                    '⚠️ Reply to an image or video *or send the command as a caption* to generate a sticker.', id);
                break;
                ...
                try {
                    ...
                    if (!targetId) {
                        await sendText(
                            ...
                            instance,
                            remoteJid,
                            '⚠️ Reply to an image to remove the background.',
                            id
                        );
                        break;
                    }
                    ...
                    if (!media?.base64 || !media.mimetype.startsWith('image/')) {
                        await sendText(server_url, apikey, instance, remoteJid,
                            '⚠️ Only images are supported.', id);
                        break;
                    }
                    ...
                    try {
                        ...
                        console.error('modern-rembg not found:', modErr.message);
                        await sendText(server_url, apikey, instance, remoteJid,
                            '❌ Function unavailable at the moment.', id);
                        break;
                    }
                    ...
                    console.error('Error in sfundo command:', err.message);
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ Failed to generate sticker without background.', id);
                }
                break;
            ...
            case 'mban':
                ...
                if (!isBotAdmin) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        '⚠️ I cannot ban members because I am not a group admin.', quotedId);
                    break;
                }
                ...
                if (targets.length > 0) {
                    ...
                    await sendText(server_url, apikey, instance, remoteJid,
                        `⚠️ Use this command by replying, mentioning or typing the number.\n\nExamples:\n• *${mainPrefix}ban @user*\n• *${mainPrefix}ban 5511999999999*\n• *${mainPrefix}ban @user @other*`, quotedId);
                    await sendReaction(server_url, apikey, instance, { remoteJid, id }, '❌');
                }
                break;
            ...
            case 'kick':
                ...
                if (!isGroup) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        '⚠️ This command can only be used in groups.', quotedId);
                    break;
                }
                ...
                if (!isBotAdmin) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        '⚠️ I cannot remove members because I am not a group admin.', quotedId);
                    break;
                }
                ...
                try {
                    ...
                    if (!groupData || !groupData.id) {
                        await sendText(server_url, apikey, instance, remoteJid,
                            '⚠️ Could not obtain group information.', quotedId);
                        break;
                    }

                    // Envia mensagem informando que o grupo está sendo desfeito
                    await sendText(server_url, apikey, instance, remoteJid,
                        '⚠️ This group is being disbanded by the admin. All participants will be removed.', quotedId);
                    ...
                    if (participantsToRemove.length === 0) {
                        await sendText(server_url, apikey, instance, remoteJid,
                            '⚠️ There are no participants to remove from the group.', quotedId);
                        break;
                    }
                    ...
                    await sendText(server_url, apikey, instance, remoteJid,
                        '🚫 *All participants have been removed from the group!*', quotedId);
            ...
            case 'apagar':
                ...
                if (!quotedMessageId || !quotedParticipant) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        '⚠️ Reply to a message so it can be deleted.', quotedId);
                    break;
                }
                ...
                try {
                    ...
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ Error trying to delete the message.', quotedId);
                    await sendReaction(server_url, apikey, instance, { remoteJid, id }, '❌');
                }
                break;
            ...
            case 'autoresposta': {
                ...
                if (quotedMessageId && argsStr.trim().length > 0) {
                    try {
                        ...
                        if (!gatilhos.length) {
                            await sendText(server_url, apikey, instance, remoteJid,
                                '⚠️ Provide at least one trigger.', quotedId);
                            break;
                        }
                        ...
                        await sendReaction(server_url, apikey, instance, { remoteJid, id }, '✅');
                        await sendText(server_url, apikey, instance, remoteJid,
                            '✅ Auto-reply added successfully.', quotedId);
                    } catch (err) {
                ...
                try {
                    ...
                    console.error('Error toggling autoresposta:', err.message);
                    await sendReaction(server_url, apikey, instance, { remoteJid, id }, '❌');
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ An error occurred while updating the command.', quotedId);
                }
                break;
            }
            ...
            case 'modobrincadeira':
                ...
                try {
                    ...
                    await sendReaction(server_url, apikey, instance, { remoteJid, id }, '❌');
                    await sendText(server_url, apikey, instance, remoteJid, `❌ An error occurred while updating the command.`, quotedId);
                }
                break;
                ...
                try {
                    ...
                    if (!botConfig) {
                        await sendText(
                            ...
                            instance,
                            remoteJid,
                            '❌ This group is not registered in the database.',
                            quotedId
                        );
                        break;
                    }
                    ...
                    const listaStatus = comandos
                        ...
                        .join('\n') || '— No command configured in this group';
                    ...
                    await sendText(
                        ...
                        instance,
                        remoteJid,
                        '❌ An error occurred while checking command status.',
                        quotedId
                    );
                }
                break;
            ...
            case 'piada':
                try {
                    ...
                    if (!piada) {
                        await sendText(server_url, apikey, instance, remoteJid,
                            '😕 I could not come up with a joke now. Please try again!', quotedId);
                        break;
                    }
                    ...
                    console.error('Error generating joke with Gemini:', err.message);
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ Error generating the joke. Try again later.', quotedId);
                }
                break;
            ...
            case 'hentai': {
                if (isGroup && isNSFW(bot)) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        '🔞 This command is disabled in this group.', quotedId);
                    break;
                }
                ...
                try {
                    if (!dono?.apikey) {
                        await sendText(server_url, apikey, instance, remoteJid,
                            '🚫 The group owner has not set an NSFW API key yet.', quotedId);
                        break;
                    }
                    ...
                    if (!resImg.data) {
                        await sendText(server_url, apikey, instance, remoteJid,
                            '⚠️ Could not get the NSFW image at the moment.', quotedId);
                        break;
                    }
            ...
            case 'stknsfw': {
                ...
                if (!dono?.apikey) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        '🚫 The group owner has not configured an NSFW API key yet.',
                        quotedId);
                    break;
                }
                ...
                for (let i = 0; i < count; i++) {
                    try {
                        ...
                        if (i === 0) {
                            await sendText(server_url, apikey, instance, remoteJid,
                                '❌ Could not generate the NSFW stickers.',
                                quotedId);
                        }
                        break;
                    }
...
╰────────────────────────────╯`;
            ...
            case 'chance': {
                if (!bot.comandos?.brincadeiras) {
                    await sendText(
                        ...
                        instance,
                        remoteJid,
                        `This command type requires fun mode to be enabled. Ask an admin or, if you are one, simply type ${mainPrefix}modobrincadeira`,
                        id
                    );
                    break;
                }
            ...
            case 'match': {
                ...
                if (idsMencionados.length < 2) {
                    await sendText(
                        ...
                        instance,
                        remoteJid,
                        'Could not find enough people for the match.',
                        id
                    );
                    break;
                }
            ...
            case 'cringe': {
                ...
                if (!dono?.apikey) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        '🚫 The group owner has not configured an API key.',
                        id);
                    break;
                }
                ...
                if (!alvoJid) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ I could not find anyone to play with you right now.',
                        id);
                    break;
                }
                ...
                for (let i = 0; i < countBrincadeira; i++) {
                    try {
                        ...
                        if (i === 0) {
                            await sendText(server_url, apikey, instance, remoteJid,
                                `❌ Could not generate the ${cleanCommand} sticker.`,
                                id
                            );
                        }
                        break;
            ...
            case 'figurinhas': {
                ...
                if (tipoArg && !mapa[tipoArg]) {
                    ...
                    await sendText(server_url, apikey, instance, remoteJid,
                        `❌ Invalid type. Use one of these: ${valids}`, id);
                    break;
                }
                ...
                if (!dono?.apikey) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        '🚫 The group owner has not configured the API key.', id);
                    break;
                }
            ...
            default:
                ...
                const info = [
                    ...
                    `👤 *User:* ${nomeUsuario}`,
                    `🆔 *ID:* ${idUsuario}`,
                    isGroup ? `👥 *Group:* ${remoteJid}` : `📞 *Private chat*`
                ].join('\n');
