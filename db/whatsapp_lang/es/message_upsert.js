...
module.exports = async function handleMessageUpsert({ data, msgOriginal, server_url, apikey, instance }) {
    ...
    function buildMenu() {
        return [
            '╭─🎲─ *MENÚ PRINCIPAL* ────',
            `│ • \`${mainPrefix}play <busqueda>\` - busca música/video`,
            `│ • \`${mainPrefix}sticker\` - crea sticker`,
            `│ • \`${mainPrefix}sfundo\` - sticker sin fondo`,
            `│ • \`${mainPrefix}tomp3\` - convierte a MP3`,
            `│ • \`${mainPrefix}ytmp4 <link>\` - descarga video de YouTube`,
            `│ • \`${mainPrefix}tiktok <link>\` - descarga video de TikTok`,
            `│ • \`${mainPrefix}instagram <link>\` - descarga medios de Instagram`,
            `│ • \`${mainPrefix}facebook <link>\` - descarga video de Facebook`,
            `│ • \`${mainPrefix}kwai <link>\` - descarga video de Kwai`,
            `│ • \`${mainPrefix}id\` - muestra el ID del chat/grupo`,
            `│ • \`${mainPrefix}piada [tema]\` - cuenta un chiste`,
            `│ • \`${mainPrefix}brincadeiras\` - lista bromas`,
            `│ • \`${mainPrefix}figurinhas\` - envía stickers`,
            `│ • \`${mainPrefix}tabela\` - muestra la tabla`,
            `│ • \`${mainPrefix}sorteio\` - crea sorteo`,
            `│ • \`${mainPrefix}sorteio2\` - sorteo sencillo`,
            `│ • \`${mainPrefix}enquete\` - crea encuesta`,
            '│',
            '│ 🔒 *Comandos de Admin*',
            `│ • \`${mainPrefix}menuadm\` - opciones administrativas`,
            '╰──────────────────────'
        ].join('\n');
    }
    ...
    // mensaje de introducción removido según solicitud
    ...
    if (isCommand) {
        ...
        switch (cleanCommand) {
            ...
            case 'menuadmin':
                ...
                const comandosAdmin = [
                    '╭──────────────',
                    '┇ *乂 M E N Ú   A D M I N 乂*',
                    '╰──────────────\n',
                    '🛠️ *GESTIÓN DEL GRUPO*',
                    `\`${mainPrefix}ban @usuario\` – Elimina al miembro mencionado`,
                    `\`${mainPrefix}apagar\` – Borra el mensaje citado`,
                    `\`${mainPrefix}permitirlink <link>\` – Añade enlace/domínio a la whitelist`,
                    `\`${mainPrefix}removerlink <link>\` – Quita enlace/domínio de la whitelist`,
                    '⚙️ *CONFIGURACIONES DEL BOT*',
                    `\`${mainPrefix}botinterage\` – Activa IA que responde en el grupo`,
                    `\`${mainPrefix}antilink\` – Borra enlaces no permitidos`,
                    `\`${mainPrefix}banextremo\` – Banea quien envía enlaces prohibidos`,
                    `\`${mainPrefix}antilinkgp\` – Borra invitaciones de otros grupos`,
                    `\`${mainPrefix}bangringos\` – Elimina quien no sea de Brasil`,
                    `\`${mainPrefix}soadm\` – Solo admins usan comandos`,
                    `\`${mainPrefix}autosticker\` – Convierte imagen/video en sticker`,
                    `\`${mainPrefix}autodownloader\` – Descarga medios automáticamente`,
                    `\`${mainPrefix}modobrincadeira\` – Activa/desactiva comandos divertidos`,
                    '🖼️ *PERSONALIZACIÓN*',
                    `\`${mainPrefix}fundomenu\` – Definir/eliminar fondo del menú`,
                    `\`${mainPrefix}fundobemvindo\` – Definir/eliminar imagen de bienvenida`,
                    `\`${mainPrefix}legendabemvindo <texto>\` – Actualizar texto de bienvenida`,
                    // agrega aquí los demás comandos administrativos…
                ].join('\n');
            ...
            case 'fundomenu':
                if (!isGroup || !isAdmin) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ Este comando solo puede ser usado por administradores en grupos.', quotedId);
                    break;
                }
                ...
                if (args[0]?.toLowerCase() === 'reset') {
                    try {
                        ...
                        await sendText(server_url, apikey, instance, remoteJid,
                            '✅ Fondo del menú eliminado. Ahora se usará el fondo predeterminado.', quotedId);
                        await sendReaction(server_url, apikey, instance, { remoteJid, id }, '♻️');
                    } catch (err) {
                        if (err.code === 'ENOENT') {
                            await sendText(server_url, apikey, instance, remoteJid,
                                '⚠️ Ningún fondo personalizado estaba definido.', quotedId);
                        } else {
                            console.error('Erro ao remover fundo do menu:', err.message);
                            await sendText(server_url, apikey, instance, remoteJid,
                                '❌ Error al intentar eliminar el fondo del menú.', quotedId);
                        }
                    }
                    break;
                }
                ...
                if (!quotedMessageId) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        `📌 Envía \`${mainPrefix}fundomenu\` respondiendo a una imagen.\nO \`${mainPrefix}fundomenu reset\` para quitar el fondo personalizado.`, quotedId);
                    break;
                }
                ...
                try {
                    ...
                    if (!media?.base64 || !media.mimetype.startsWith('image/')) {
                        await sendText(server_url, apikey, instance, remoteJid,
                            '⚠️ El mensaje respondido no contiene una imagen válida.', quotedId);
                        break;
                    }
                    ...
                    await sendText(server_url, apikey, instance, remoteJid,
                        '✅ Fondo del menú actualizado con éxito!', quotedId);
                    await sendReaction(server_url, apikey, instance, { remoteJid, id }, '🖼️');

                } catch (err) {
                    console.error('Erro ao salvar novo fundo do menu:', err.message);
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ Error al intentar guardar la nueva imagen de fondo.', quotedId);
                }
                break;
            ...
            case 'fundobemvindo':
                ...
                try {
                    ...
                    if (!bot) {
                        await sendText(server_url, apikey, instance, remoteJid, '❌ Bot no encontrado en la base de datos.', quotedId);
                        break;
                    }
                    ...
                    if (args[0]?.toLowerCase() === 'reset') {
                        ...
                        await sendText(server_url, apikey, instance, remoteJid,
                            '♻️ Imagen de bienvenida eliminada. Se usará la imagen predeterminada.', quotedId);
                        await sendReaction(server_url, apikey, instance, { remoteJid, id }, '✅');
                        break;
                    }
                    ...
                    if (!quotedMessageId) {
                        await sendText(server_url, apikey, instance, remoteJid,
                            `📎 Envía \`${mainPrefix}fundobemvindo\` respondiendo a una imagen.\nO usa \`${mainPrefix}fundobemvindo reset\` para quitarla.`, quotedId);
                        break;
                    }
                    ...
                    await sendText(server_url, apikey, instance, remoteJid,
                        '✅ Imagen de bienvenida actualizada con éxito!', quotedId);
                    await sendReaction(server_url, apikey, instance, { remoteJid, id }, '🖼️');

                } catch (err) {
                    console.error('Error en fundobemvindo:', err.message);
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ Error al procesar el comando de fondo de bienvenida.', quotedId);
                }
                break;
            ...
            case 'bemvindo':
                if (!isGroup || !isAdmin) {
                    await sendReaction(server_url, apikey, instance, { remoteJid, id }, '❌');
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ Solo administradores pueden activar o desactivar esta función.', quotedId);
                    break;
                }
                ...
                if (!allowedCommands.bemvindo) {
                    ...
                    await sendText(
                        ...
                        instance,
                        remoteJid,
                        '⚠️ Esta función no está disponible en tu plan actual. Accede al panel y actualiza el plan del bot.',
                        quotedId
                    );
                    break;
                }
                ...
                try {
                    const bot = await BotConfig.findOne({ groupId: remoteJid });
                    if (!bot) {
                        await sendText(server_url, apikey, instance, remoteJid, '❌ Grupo no encontrado en la base de datos.', quotedId);
                        break;
                    }
                    ...
                    await sendText(server_url, apikey, instance, remoteJid,
                        `✅ Función de bienvenida ${novoStatus ? '*activada*' : '*desactivada*'} con éxito.`, quotedId);
                    await sendReaction(server_url, apikey, instance, { remoteJid, id }, novoStatus ? '✅' : '🛑');

                } catch (err) {
                    console.error('Error al alternar bemvindo:', err.message);
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ Error al activar/desactivar la función de bienvenida.', quotedId);
                }
                break;
            ...
            case 'legendabemvindo':
                ...
                try {
                    ...
                    if (args[0]?.toLowerCase() === 'reset') {
                        ...
                        await sendText(server_url, apikey, instance, remoteJid,
                            '✅ Leyenda de bienvenida borrada con éxito.', quotedId);
                        await sendReaction(server_url, apikey, instance, { remoteJid, id }, '🗑️');
                        break;
                    }
                    ...
                    if (!novaLegenda) {
                        await sendText(server_url, apikey, instance, remoteJid,
                            `✏️ Usa \`${mainPrefix}legendabemvindo <texto>\` o responde a un mensaje con el texto deseado.\nEjemplo: \`${mainPrefix}legendabemvindo ¡Bienvenido(a) al grupo! 🎉\``, quotedId);
                        break;
                    }
                    ...
                    await sendText(server_url, apikey, instance, remoteJid,
                        '✅ Leyenda de bienvenida actualizada con éxito!', quotedId);
                    await sendReaction(server_url, apikey, instance, { remoteJid, id }, '✍️');

                } catch (err) {
                    console.error('Error al actualizar leyenda de bienvenida:', err.message);
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ Error al actualizar la leyenda de bienvenida.', quotedId);
                }
                break;
            ...
            case 'permitirlink':
                ...
                try {
                    ...
                    await sendText(server_url, apikey, instance, remoteJid, '❌ Error al agregar el enlace.', quotedId);
                }
                break;
            ...
            case 'removerlink':
                ...
                try {
                    ...
                    await sendText(server_url, apikey, instance, remoteJid, '❌ Error al eliminar el enlace.', quotedId);
                }
                break;
            case 'linkspermitidos':
                if (!isGroup) {
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ Este comando solo se puede usar dentro de grupos.', quotedId);
                    break;
                }
                ...
                try {
                    ...
                    if (!bot) {
                        await sendText(server_url, apikey, instance, remoteJid,
                            '❌ Grupo no encontrado en la base de datos.', quotedId);
                        break;
                    }
                    ...
                    if (links.length === 0) {
                        await sendText(server_url, apikey, instance, remoteJid,
                            'ℹ️ Ningún enlace o dominio fue liberado aún en este grupo.', quotedId);
                        break;
                    }

                    const listaFormatada = links.map((link, i) => `*${i + 1}.* \`${link}\``).join('\n');

                    const resposta = `📜 *Enlaces y dominios permitidos en el grupo:*\n\n${listaFormatada}`;

                    await sendText(server_url, apikey, instance, remoteJid, resposta, quotedId);
                    await sendReaction(server_url, apikey, instance, { remoteJid, id }, '📃');
                } catch (err) {
                    console.error('Error al listar enlaces permitidos:', err.message);
                    await sendText(server_url, apikey, instance, remoteJid,
                        '❌ Error al listar los enlaces permitidos.', quotedId);
                }
                break;
            ...
            case 'status':
                ...
                if (!isGroup) {
                    await sendText(
                        ...
                        instance,
                        remoteJid,
                        '❌ Este comando solo se puede usar dentro de grupos.',
                        quotedId
                    );
                    break;
                }
